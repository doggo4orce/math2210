...
\exercise Prove that the cube of any integer $a$ is of the form $9k, 9k+1$, or $9k+8$ for some $k\in\INTEGER$.
\vsp
...
\exercise If $n\in\INTEGER$, what are the possible values for
\begin{enumerate}[(a)]
\item $\GCD{n, n+2}$?
\item $\GCD{n, n+6}$?
\item $\GCD{n, n+1}$?
...
\exercise Suppose $a,b,c\in\INTEGER$.
\begin{enumerate}[(a)]
...
\exercise Prove that an integer $n>1$ is composite if and only if there exists $m,k\in\INTEGER$ with $1 < m < n$ and $1 < k < n$ such that $n = mk$.
\vsp
...
\notation As usual, we express repeated multiplication with exponents.  For any $z\in\COMPLEX$ and any integer $n\geq 0$,
\begin{center}
...
\exercise Show that $-i$ is also a solution to the equation $z^{2} = -1$.
\vsp
...
\exercises Prove each of the following for $z\in\COMPLEX$ and $m,n\in\INTEGER$.
\begin{enumerate}[(a)]
\item $z^{m}z^{n} = z^{m+n}$
\item $z^{m}/z^{n} = z^{m-n}$
...
\p This definition is actually based upon the familiar technique of \emph{multiplication by the conjugate}.
\begin{definition}
Let $z = a + bi\in\COMPLEX$.  The \DEF{complex conjugate} of $z$, denoted $\CC{z}$, is defined by $\CC{z} = a - bi$.
\end{definition}
\vsp

\p Note that for any $z = a + bi\in\COMPLEX$, we have $z\CC{z} = (a + bi)(a - bi) = a^2 - abi + bia - bi^2 = a^2 + b^2$.  In particular, $z\CC{z}\in\REAL$.  This idea is used to give meaning to \emph{division} of complex numbers, using an analogue to the familiar technique known as 
...
\p Negative powers are defined by $z^{n} = \dfrac{1}{z^{-n}}$, whenever $n$ is an integer with $n\leq 1$.
\vsp
...
\subsection{Polar Coordinate Representations}

-

\subsection{Roots of Unity}

\p -definition of root of unity
\p -principle root of unity
\p -discussion of the $\sqrt{bullet}$ symbol
\p -quadratic formula
\end{document}
