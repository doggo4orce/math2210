...
\newcommand{\coursetopics}{
	\begin{enumerate}
		\item Chapter 1 - Arithmetic in $\mathbb{Z}, \mathbb{Q}, \mathbb{R}$, and $\mathbb{C}$
		\begin{itemize}
			\item	1.1 - The Division Algorithm
			\item	1.2 - Divisibility
			\item   1.3 - Primes and Unique Factorization
			\item   1.4 - $\mathbb{Q}, \mathbb{R}$, and $\mathbb{C}$
		\end{itemize}
		
		\item Chapter 2 - Complex Numbers
		\begin{itemize}
			\item	2.1 - Arithmetic in $\mathbb{C}$  
			\item	2.2 - Euler's Formula 
			\item	2.3 - The Complex Plane 
			\item	2.4 - Arguments
			\item   2.5 - Roots of Unity 
		\end{itemize}
		
		\item Chapter 3 - Congruence in $\mathbb{Z}$ and Modular Arithmetic 
		\begin{itemize}
			\item	3.1 - Congruence and Congruence Classes 
			\item	3.2 - Modular Arithmetic 
			\item	3.3 - The Structure of $\mathbb{Z}_{p}$ when $p$ is Prime
		\end{itemize}
		
		\item Chapter 4 - Rings  
		\begin{itemize}			
			\item   4.1 - Binary Operations
			\item	4.2 - Isomorphic Binary Structures   
			\item   4.3 - Definition and Examples of Rings
			\item	4.4 - Integral Domains and Fields
			\item	4.5 - Cancellation Properties
			\item   4.6 - Ring Homomorphisms
		\end{itemize}
		
		\item Chapter 4 - Arithmetic in $F[x]$
		\begin{itemize}			
			\item   4.1 - Polynomial Rings
			\item	4.2 - Divisibility in $F[x]$   
			\item   4.3 - Irreducibility and Unique Factorization of Polynomials
			\item	4.4 - Polynomial Functions, Roots and Reducibility
			\item	4.5 - Irreducibility in $\mathbb{Q}[x]$
			\item   4.6 - Irreducibility in $\mathbb{R}[x]$ and $\mathbb{C}[x]$
		\end{itemize}
		
		\item Chapter 5 - Congruence in $F[x]$ and Congruence Class Arithmetic 
		\begin{itemize}
			\item	5.1 - Congruence in $F[x]$ and Congruence Classes 
			\item	5.2 - Arithmetic in $F[x]/(p(x))$ 
			\item	5.3 - The Structure of $F[x]/(p(x))$ when $p(x)$ is Irreducible
		\end{itemize}
