...
\item Let $R$ be an integral domain and assume that the division algorithm holds.  That is, assume for every $f(x),g(x)\in R[x]$ with $g(x)$ non-zero, there exists $q(x), r(x)\in R[x]$ such that $f(x) = q(x)g(x) + r(x)$ and either $r(x) = 0_{R}$ or $0 \leq\DEG{r(x)} < \DEG{g(x)}$.  Prove that $F$ is a field.
\end{enumerate}
\vsp
