...
\p {\huge \S5.6 Problems}
\vsp

\begin{enumerate}[1.]
\item Find all roots of the polynomial $f(x) = x^4 - 4x^3 + 3x^2 + 14x + 26\in\C[x]$, given that $3 + 2i$ is a root of $f(x)$.

\item Find a polynomial in $\R[x]$ that satisfies the given conditions.
\begin{enumerate}[(a)]
\item monic of degree $3$ with $2$ and $3 + i$ as roots
\item monic of minimal degree wth $1-i$ and $2i$ as roots
\item monic of minimal degree with $3$ and $4i - 1$ as roots
\end{enumerate}

\item Factor each polynomial as a product of irreducible polynomials in $\Q[x]$, $\R[x]$, and $\C[x]$:
\begin{enumerate}[(a)]
\item $x^4 - 2$
\item $x^3 + 1$
\item $x^3 - x^2 - 5x + 5$
...
\item Show that a polynomial of odd degree in $\R[x]$ with no repeated roots must have an odd number of real roots.

\item Give an example of a polynomial $f(x)\in\C[x]$ and $\lambda\in\C$ such that $\lambda$ is a root of $f(x)$ but $\CC{\lambda}$ is not.
\end{enumerate}
